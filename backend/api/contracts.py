...
def init_sample_contracts():
    if not contracts_db:  # Only initialize if empty
        # Import vessels to get names
        from api.vessels import vessels_db
...
def create_contract():
    ...
    try:
        ...
        required_fields = ["vesselId", "vesselName", "energyAmount", "pricePerKwh", "startTime", "endTime", "terms"]
...
def complete_contract(contract_id: str):
    ...
    try:
        ...
        if contract.status not in [ContractStatus.PENDING.value, ContractStatus.ACTIVE.value]:
            return jsonify({"error": "Only pending or active contracts can be completed"}), 400
