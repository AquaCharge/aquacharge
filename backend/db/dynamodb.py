...
class DynamoDBClient:
    ...
    def __init__(self):
        ...
        self.users_table_name = os.environ.get(
            "DYNAMODB_USERS_TABLE", "aquacharge-users-dev"
        )
        self.stations_table_name = os.environ.get(
            "DYNAMODB_STATIONS_TABLE", "aquacharge-stations-dev"
        )
        self.chargers_table_name = os.environ.get(
            "DYNAMODB_CHARGERS_TABLE", "aquacharge-chargers-dev"
        )
        self.vessels_table_name = os.environ.get(
            "DYNAMODB_VESSELS_TABLE", "aquacharge-vessels-dev"
        )
        self.bookings_table_name = os.environ.get(
            "DYNAMODB_BOOKINGS_TABLE", "aquacharge-bookings-dev"
        )
        self.contracts_table_name = os.environ.get(
    ...
    def prepare_item_for_dynamodb(item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in item.items():
            if value is None:
                ...
                prepared[key] = [
                    (
                        DynamoDBClient.prepare_item_for_dynamodb(item)
    ...
    def scan_table(
        self, table_name: str, filter_expression=None, limit: int = 100
    ) -> List[Dict[str, Any]]:
    ...
    def get_user_by_email(self, email: str) -> Optional[Dict[str, Any]]:
        ...
        results = self.query_index(
            self.users_table_name, "email-index", Key("email").eq(email)
        )
    ...
    def get_users_by_org(self, org_id: str) -> List[Dict[str, Any]]:
        ...
        return self.query_index(
            self.users_table_name, "orgId-index", Key("orgId").eq(org_id)
        )
    ...
    def get_stations_by_city(self, city: str) -> List[Dict[str, Any]]:
        ...
        return self.query_index(
            self.stations_table_name, "city-index", Key("city").eq(city)
        )

    def get_stations_by_status(self, status: int) -> List[Dict[str, Any]]:
        """Get stations by status using status-index GSI"""
        return self.query_index(
            self.stations_table_name, "status-index", Key("status").eq(status)
        )
    ...
    def get_vessels_by_user(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        return self.query_index(
            self.vessels_table_name, "userId-index", Key("userId").eq(user_id)
        )
    ...
    def get_bookings_by_user(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        return self.query_index(
            self.bookings_table_name, "userId-index", Key("userId").eq(user_id)
        )
    ...
    def get_contracts_by_vessel(self, vessel_id: str) -> List[Dict[str, Any]]:
        ...
        return self.query_index(
            self.contracts_table_name, "vesselId-index", Key("vesselId").eq(vessel_id)
        )
