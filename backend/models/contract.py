...
class Contract(BaseModel):
    ...
    def from_dict(cls, data: Dict[str, Any]):
        ...
        if isinstance(data.get('startTime'), str):
            data['startTime'] = datetime.fromisoformat(data['startTime'].replace('Z', '+00:00'))
        if isinstance(data.get('endTime'), str):
            data['endTime'] = datetime.fromisoformat(data['endTime'].replace('Z', '+00:00'))
    ...
    def to_public_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "endTime": self.endTime.isoformat() if isinstance(self.endTime, datetime) else self.endTime,
            "status": self.status,
            "terms": self.terms,
            "createdAt": self.createdAt.isoformat() if isinstance(self.createdAt, datetime) else self.createdAt,
            "updatedAt": self.updatedAt.isoformat() if self.updatedAt and isinstance(self.updatedAt, datetime) else self.updatedAt,
