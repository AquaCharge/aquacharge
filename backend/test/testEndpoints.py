...
def test_update_charger(client):
    rv = client.put("/api/chargers/charger-002", json={"active": False, "maxRate": 60.0})
    assert rv.status_code == 200
...
def test_update_station(client):
    rv = client.put("/api/stations/station-002", json={"city": "San Jose", "status": "MAINTENANCE"})
    assert rv.status_code == 200
...
def test_update_user(client):
    rv = client.put("/api/users/user-003", json={"email": "newmail@example.com", "active": False})
    assert rv.status_code == 200